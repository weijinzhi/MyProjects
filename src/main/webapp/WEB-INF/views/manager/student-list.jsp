...
<body>
	<div id = "queryDiv">
		<input id = "studentId" type="text" placeholder="请输入学生学号" >
		<button id = "queryButton" class="btn btn-primary" type="button">查询</button>
	</div>
	<form id="form1">
		<table class="table table-bordered" id = 'tableResult'>
			<caption>查询学生结果</caption>
			<thead>
				<tr>
					<th>序号</th>
					<th>ID</th>
	...
	<script type='text/javascript'>    
        ...
        var options = {  
            ...
            onPageClicked: function (e, originalEvent, type, page) {  
            	var studentName = $("#studentName").val(); //取内容
            	buildTable(studentName,page,PAGESIZE);//默认每页最多10条
            }  
        }  
        ...
        //生成表格
        function buildTable(studentName,pageNumber,pageSize) {
        	var url =  urlRootContext + "/student/list"; //请求的网址
             var reqParams = {'studentName':studentName, 'pageNumber':pageNumber,'pageSize':pageSize};//请求数据
             $(function () {   
             	  $.ajax({
             	        type:"POST",
             	        url:url,
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
            	 $("#tableBody").append("<tr><td>" +this.id +
            			 "</td><td>" +this.studentId + "</td><td>" + 
            			 this.studentName + "</td><td>" + 
            			 this.password + "</td><td>" + 
            			 this.sex + "</td><td>" + 
            			 this.classnum + "</td><td>" + 
            			 this.academy + "</td></tr>");
        ...
        $(function() {
            ...
            $("#queryButton").bind("click",function(){
            	...
            	buildTable(studentName,1,PAGESIZE);
            });
        });
    </script>
</body>
